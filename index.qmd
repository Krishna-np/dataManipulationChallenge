...
`shipments_df.dtypes` shows actualShipDate is `datetime64[ns]`. Both date columns must share the same `dtype` — subtracting a string from a datetime throws a `TypeError`, making `days_late` impossible to calculate.
:::

::: {.callout-tip}
## Your Answer: String vs Date Comparison
String comparison is character-by-character, not chronological. "04-11-2025" > "05-20-2024" returns False as strings because "4" < "5" at the month position, despite April 2025 being later than May 2024. As datetimes, it correctly returns True.
:::

::: {.callout-tip}
## Your Answer: Debug This Code
Two problems: `is_late` is being created in this same `.assign()` call, so referencing `shipments_df['is_late']` throws a `KeyError`. And even if it existed, a plain if on a Series raises a `ValueError` — Python can't evaluate an entire column as a single boolean. The fix:
```python
...
Both filter to the same result, but `.query('is_late == True')` is cleaner inside a method chain — no need to repeat the dataframe name or wrap it in a lambda. Boolean indexing like `df[df['is_late'] == True]` works fine but adds visual noise, especially in longer chains.
:::

::: {.callout-tip}
## Your Answer: Additional Row Querying
The `@` symbol lets `.query()` reference a local variable rather than a column name:
```python
late_threshold = 5
late_shipments = (
    shipments_with_lateness
...
```
Adjusting `late_threshold` updates the filter without touching the query string itself.
:::
...
`.drop(columns=['quantity'])` makes sense when removing a small number of columns from a wide dataset. `.filter()` is better when you only need a handful of columns — listing 3 to keep is cleaner than listing 15 to drop. In method chains, `.filter()` also makes it immediately clear which columns are moving forward.
:::

::: {.callout-tip}
## Your Answer: Handling Missing Data
`.dropna()` without subset removes any row with a missing value in any column, which can silently shrink the dataset more than expected. `.dropna(subset=['plannedShipDate', 'actualShipDate'])` targets only the columns critical to the analysis, preserving rows that are still usable even if other fields are incomplete.
:::
...
`ascending=True` sorts from smallest to largest (A→Z, earliest→latest), while ascending=False flips it. To sort by multiple columns, pass a list to `sort_values()`:
```python
.sort_values(['is_late', 'days_late'], ascending=[False, False])
```
This sorts by `is_late` first, then breaks ties using `days_late` — so all late shipments appear at the top, ordered by how late they are.
:::

::: {.callout-tip}
## Your Answer: Index Management
When you sort a dataframe, the original row index travels with each row — so instead of 0, 1, 2, 3... you end up with something like 0, 45, 12, 387. This becomes problematic when you need to reference rows by position or when the index is used in subsequent operations. `.reset_index(drop=True)` reassigns a clean sequential index. The `drop=True` part prevents the old index from being added as a new column.
:::
...
In pandas, `True` is treated as `1` and `False` as `0`, so `sum()` on a boolean column counts the number of `True` values — in this case, the total number of late shipments. `mean()` then gives the proportion of late shipments, which is why multiplying by `100` converts it directly to a percentage.
:::
...
`how='left'` keeps all rows from the left dataframe regardless of whether a match exists in the right one — which is what you want here, since every shipment should be preserved even if its `partID` isn't found in `product_line_df`. To check if any shipments were lost, compare row counts before and after the merge, or check for nulls in the `prodCategory` column:
```python
shipments_with_category['prodCategory'].isna().sum()
```
:::

::: {.callout-tip}
## Your Answer: Key Column Matching
If `product_line_df` has duplicate `partID` values, the merge will create multiple rows for each matching shipment — one for every duplicate. This inflates the dataset and can skew any aggregations done afterwards. It's worth checking for duplicates before merging:
```python
...
`.groupby('prodCategory')` splits the dataframe into separate groups based on unique values in `prodCategory` — think of it as creating a mini-dataframe for each category behind the scenes. `.agg()` is then needed to specify what calculation to perform on each group. Without it, the groupby object just sits there — pandas doesn't know what to do with the groups until you tell it.
:::

::: {.callout-tip}
## Your Answer: Multi-Level Grouping
Grouping by `['shipID', 'prodCategory']` answers a different question than grouping by `'prodCategory'` alone. Instead of asking "how does lateness vary across categories?", it asks "for each shipment, how did each category within that shipment perform?" This is useful because a single `shipID` can have multiple rows with different `partIDs`, and those parts may belong to different categories — so grouping by both lets you analyze performance at the shipment-category level rather than just the category level.
:::
...
This analysis answers how often shipments contain products from more than one category. Grouping by `['shipID', 'prodCategory']` works at the shipment-category level, revealing which categories appeared together in the same shipment — something grouping by `'prodCategory'` alone can't tell you. Knowing `23.3%` of shipments are multi-category is useful for management because it affects warehouse picking, packaging logistics, and whether a late item in one category is impacting customers who ordered across multiple categories.
:::
...
on_time_by_category = (
    ...
    .mul(100)
    .reset_index()
    .rename(columns={'is_late': 'on_time_rate'})
    .sort_values('on_time_rate', ascending=False)
)

fig, ax = plt.subplots(figsize=(8, 5))
sns.barplot(
    data=on_time_by_category,
    x='prodCategory',
    y='on_time_rate',
    hue='prodCategory',
    palette='Blues_d',
...
ax.set_title('On-Time Delivery Rate by Product Category', fontsize=14, fontweight='bold')
ax.set_xlabel('Product Category')
ax.set_ylabel('On-Time Rate (%)')
ax.set_ylim(80, 100)
ax.axhline(y=on_time_by_category['on_time_rate'].mean(), color='red', linestyle='--', label='Overall Average')
ax.legend()
plt.tight_layout()
plt.show()
```
